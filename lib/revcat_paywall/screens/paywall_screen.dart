...
class _PaywallScreenState extends State<PaywallScreen> {
  ...
  void initState() {
    ...
    _loadAllPackages();
    SubscriptionService()
        .refresh(); // ✅ Ensure SubscriptionService is up-to-date
  }
  ...
  Future<void> _purchase(Package package) async {
    try {
      ...
      if (entitlements.isNotEmpty) {
        await SubscriptionService().refresh(); // Refresh access flags
        await LocalFlags.setTasterTrialUsed(true);

        if (!mounted) return;
        context.go('/upgrade-success');
      } else {
        throw Exception('No active entitlement found after purchase.');
      }
